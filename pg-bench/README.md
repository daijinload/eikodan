...
RAMディスクは `fsync=off` を入れれば速度差≒0(効くのはチューニングと "VMを挟まないこと")。
...
- **同一 Docker 内なら native 並み**: pgbench で docker **26,561 tps**(VM内・socket)vs **4,389 tps**(ホストから)。
  → 詳細は [POSTGRES_BENCHMARK.md](./POSTGRES_BENCHMARK.md) の「わかったこと #6」。
