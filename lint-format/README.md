...
> ここで出した「lint/format ツールの選定」の結論は本体 [`lastshot/`](../lastshot/) に取り込み済みで、
...
| SQL              | `samples/query.sql`              | **sqlfluff**               | **sqlfluff**                     | brew（Python・依存ごと配布）         |

## 使い方

```sh
bash setup-lint.sh   # ツール取得（rustfmt/clippy 確認、oxfmt を .lint-tools にローカル固定、buf/shfmt/shellcheck/sqlfluff を brew）
...
- **SQL は sqlfluff（format + lint 兼用）**。oxfmt は SQL 非対応なので別建て。`sqlfluff lint` は整形逸脱も
  ...
  方言は `pg-bench` に合わせ **postgres** 固定（`.sqlfluff`）。Python 製だが brew が依存ごと配布する。
