...
step "[1/5] rustfmt（cargo fmt --all --check）"
(cd samples/rust && cargo fmt --all --check) || fail=1

step "[2/5] clippy（-D warnings）"
(cd samples/rust && cargo clippy --all-targets -- -D warnings) || fail=1

step "[3/5] oxfmt --check（TOML/YAML/Markdown/CSS/HTML）"
if [[ -n "$OXFMT" ]]; then
  "$OXFMT" --check . || fail=1
else
...
if [[ "$fail" -eq 0 ]]; then
  ...
  echo "NG: 上の指摘を解消すること（整形は各ツールの --write/-w / 'buf format -w' / 'shfmt -w' で適用）"
fi
exit "$fail"
