...
- **`CARGO_INCREMENTAL=0` は規模に関わらずフル再ビルドを大きく速くする**（30 で -52% / 90 で -39%）。フル再ビルドが多い人 / CI 向け。代償は日常ループの税（check +0.1〜0.2s / build +0.2〜0.5s、規模が増えるほど増える）で、常時 local には過剰。
...
- **sccacheは現状入れていない（fastweb 側 ＝ lastshot 側は `.cargo/config.toml` に `rustc-wrapper = "sccache"` 入り）**。入れるなら `RUSTC_WRAPPER=sccache` だけ・incrementalは残す（④）が、**新構成（全 opt-0）では旨味は当時より小さい**: 30 クレートでフル再ビルド -13%、90 クレートでは **incr ON が baseline よりむしろ遅くなる**（+14%・依存節約を sccache 往復が食う）。`CARGO_INCREMENTAL=0` まで切れば規模に関わらず -40〜52% 速くなるが、日常ループへ税（check +0.1〜0.2s / build +0.2〜0.5s）を払う。**最大の旨味はCI/共有キャッシュ**（マシンをまたいで依存を使い回す）── 手元の効きは規模次第なので一度測ってから決める。
