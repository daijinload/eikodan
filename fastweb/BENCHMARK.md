...
### 計測 (lastshot, 2026-06-15、背景プロセス静穏時、sccache 毎回 purge)

- ハーネス: 各構成について `sccache --stop-server && rm -rf ~/Library/Caches/Mozilla.sccache`（**毎回 sccache 全消し**）→
  Cargo.toml / RUSTFLAGS / RUSTUP_TOOLCHAIN を構成に設定 → `cargo clean -q && /usr/bin/time -p cargo build [--release] -p app` を **5 試行**
  （trial 1 = **true cold**、trial 2-5 = warm sccache、**warm は 4 試行の median**）
- sccache 有効（`.cargo/config.toml` の `rustc-wrapper = "sccache"`）、他設定は構成で切替
...
- 構成ごとに sccache を空にしているので、**trial 1 は構成本来の true cold（前段の残骸の影響を受けない）**

| 構成 | true cold (trial 1) | warm trials (2-5) | warm median | warm spread (max-min) |
|---|---|---|---|---|
| **A: dev (nightly + cranelift + -Z threads=8 + lld + opt-0)** | **10.58s** | 5.91 / 5.99 / 6.11 / 6.46 | **~6.05s** | 0.55s |
| **B: dev (stable + RUSTFLAGS="" + opt-0)** | **12.79s** | 5.66 / 5.87 / 5.88 / 5.67 | **~5.77s** | 0.22s |
| **C: release (stable + RUSTFLAGS="" + opt-3)** | **20.09s** | 6.31 / 6.33 / 6.28 / 6.17 | **~6.30s** | 0.16s |

### 過去の計測との関係

以前の本節は sccache を purge せず「他構成で焼いたエントリが残った状態」での cold を「cold」と呼んでいた
（lastshot で日常的に何か焼いた後の cold 状態に相当）。それだと前段でどの構成を焼いたかで sccache の hit/miss
分布が変わり、構成本来の cold 差が見えない（実際、以前の cold は 12〜14s に潰れていた）。今回は構成ごとに
sccache を空にしてから測ったので、**cold での構成差が本来の姿で出ている**（true cold で A 10.58s ／ B 12.79s
／ C 20.09s と大きく開く）。

### 読み方

**(a) warm が普段の体感、true cold は節目だけ**

普段の dev ループは sccache warm 状態で回っているので、**フルビルドの体感は約 5.8〜6.3s**（warm spread 0.16〜0.55s）。
true cold は新規 clone / `cargo clean` + sccache が空 / 別 toolchain で初めて焼く / 大きい deps の更新で当たる。
日常で「`cargo clean` 直後」を踏んでも sccache が他作業のエントリを持っていれば実体は cold ではなく半 warm
（前段構成と同 toolchain・同 opt-level なら proc-macro / dep を hit する）なので、true cold を踏むのは
本当に節目だけ。

**(b) warm でも dev opt-0 < release opt-3、差は約 0.5s (~9%)**

warm median は B 5.77s / C 6.30s で release が +0.5s 程度遅い。重い依存の opt-3 LLVM コード生成は sccache が
返すが、自前クレートだけは opt-3 で本気で LLVM が回る分が残る。ただし差は 0.5s 程度 ＝ 構成決定に影響しない
レベル（**`./run release` の体感が dev とほぼ変わらない**の根拠）。

**(c) warm では nightly フル構成 vs stable+素のツール は ±5%、A の方がむしろ気持ち重い (6.05s vs 5.77s)**
...
**(d) true cold では構成差が大きく出る (A 10.58s / B 12.79s / C 20.09s)**

- **nightly vs stable (同 opt-0、A vs B): nightly が -17% 速い (10.58s vs 12.79s)**。sccache が空で本物の rustc が
  全部走る局面では `-Z threads=8` の並列フロントエンドがコア余りを並列で食って効く。「真の cold」のときだけ
  nightly が気持ちいい、というのは事実として残る。
- **opt-0 vs opt-3 (同 stable、B vs C): release が +57% 重い (12.79s vs 20.09s)**。opt-3 LLVM の本気仕事を
  sccache が返せないので、~7s ぶん実 LLVM 最適化のコストが乗る。release 検証を「sccache の世代交代直後」に
  踏むと意外と重い、を覚えておくと事故を避けられる（普段は warm で 6.30s で済むので気にならない）。
- 普段のループは warm（sccache 有効）なのでこの true cold 差は乗らないが、CI / 新規 clone / worktree 初回・
  toolchain 切替後の最初の 1 ビルドだけは構成依存が露わになる。

### 追加計測: `CARGO_INCREMENTAL=0` + 差分ビルド (dev 2 構成、各 5 cold + 5 warm + 5 incr)

上の表は cargo の既定 `incr=ON` 前提。④ で「フル再ビルドが多い環境では `incr OFF` が -39〜-52%」と結論したので、
lastshot 規模で本当に同じ向きか、また hot loop の差分ビルド (`touch 1 feature → cargo build -p app`) でも
同じ向きが保たれるかを別途取った。

**ハーネス**: 各構成について以下を順に実行（`CARGO_INCREMENTAL=0` を全モードで設定）:
1. **5 cold**: 各 trial ごとに `sccache --stop-server && rm -rf ~/Library/Caches/Mozilla.sccache && cargo clean && time cargo build -p app`
2. **5 warm**: cold 5 trial 直後の sccache 状態のまま `cargo clean && time cargo build -p app` を 5 回
3. **5 incremental**: warm 直後の target/ 状態から `touch crates/feature-counter/src/lib.rs && time cargo build -p app` を 5 回

| 構成 | cold T1 (※) | cold T2-5 median | warm median | incr median |
|---|---|---|---|---|
| **A: dev nightly + cranelift + -Z threads + lld + opt-0** | 14.56s | **10.88s** (spread 0.61s) | **5.79s** (spread 1.57s ※外) | **0.78s** (spread 0.02s) |
| **B: dev stable + RUSTFLAGS="" + opt-0** | 16.49s | **12.86s** (spread 0.54s) | **5.48s** (spread 0.30s) | **0.63s** (spread 0.02s) |
| Δ (A vs B) | A が -1.93s (-12%) | A が -1.98s (**-15%**) | B が -0.31s (-5%、誤差圏) | **B が -0.15s (-19%)** |

※ cold T1 は sccache + OS page cache (rustc/cargo registry/stdlib/dep ソース) が両方 cold な「真の初回」。
T2-5 は sccache を毎回 purge しているが OS page cache は warm なので +3.5〜3.7s 安い。
**新規 clone 直後の体感** ≈ T1、**`cargo clean && sccache --clear` 後の体感** ≈ T2-5。どちらも「cold」だが場面が違う。

※外 A warm trial 5 = 7.18s は他セッションでも再現する A 特有の外れ値（trial 1-4 は 5.6〜5.8s 圏内、median は
trial 5 の影響を受けにくい）。

### この追加計測で見えたこと

**(f) cold では nightly+full が -12〜15% 速い（前 (d) と整合）**

T1 同士で -12%、T2-5 median 同士で -15%。`-Z threads=8` の並列フロントが dep フルコンパイルで効く ── 真の cold で
だけ nightly チューニングが効く、を再現確認。

**(g) warm はほぼ誤差圏（前 (c) と整合）、incr=OFF にすると stable がさらにわずかに速い**

A warm median 5.79 (外れ除き 5.71) / B warm median 5.48、B が -5%。前回計測 (incr=ON) でも B が -5% 程度速かった
方向と同じ。warm では nightly チューニングは効かない、stable+plain で十分という結論を維持。

**(h) 差分ビルド (hot loop) では stable+plain のほうが -19% 速い (0.63s vs 0.78s)**

これは今回の新発見。`touch 1 feature → cargo build -p app` の hot loop ── 1 feature クレートの再コンパイル +
app の再リンク ── では **nightly+cranelift より stable+LLVM のほうが 0.15s 速い**。

理由の候補:
- 単一の小クレート再コンパイルでは `-Z threads=8` の並列フロントは効かない（クレート内並列化の余地が小さい）
- cranelift は codegen を高速化する代わりに最適化が弱く、出力 .rlib / バイナリが大きめになる ── 後段の link が
  わずかに重くなる可能性
- stable+LLVM のほうが cargo / rustc / sccache の組み合わせで枯れていて、startup / fingerprint 周りのオーバー
  ヘッドが小さい可能性
- 0.15s は絶対値としては小さく、複合要因。深追いは費用対効果が悪い
...
warm は 5.48〜5.79s で、上の incr=ON の warm median (B 5.77 / A 6.05) から **A は -0.4s, B は -0.3s 速くなった**。
incr=OFF にすると workspace クレートも sccache が cache できる（`Non-cacheable reasons: incremental` 解除）ので、
warm hit がワークスペース全体に広がる。
ただし true cold は incr=OFF でむしろ +3〜4s 遅くなる（T2-5 median で A 10.88 / B 12.86 vs incr=ON cold A 10.58 /
B 12.79）── sccache の store 対象が広がる分のオーバーヘッドが cold で乗ると見られる。incr=OFF を採用する/しないは
**「cold が多い環境（CI / cargo clean 多用）」と「warm が多い環境（日常開発）」のどちらに合わせるか**で決まる。
lastshot は warm が支配的なので **incr ON が既定（変更なし）**。④ の結論 ── 「`incr OFF` の旨味は『フル再ビルドが
多い環境』、日常ループでは税」── と整合。

**(e) "warm ~6s で収まる" は feature 分割が要らないという話ではない**

(b)(c) で「warm では構成差が小さい（dev / release / nightly / stable で 5.8〜6.3s に収まる）」を見ると
「じゃあ feature 分割の build-speed 動機も弱いのでは？」と読まれかねないので、明示的に分解しておく。
**~6s で収まるのはフルビルド warm の場面だけで、日々の hot loop ＝ 増分ビルド と `check -p` では feature
分割が依然 1 桁効いている**。3 場面で並べると:
...
- **warm sccache（普段の体感）= 約 5.8〜6.3s**
  - dev opt-0 ~5.8〜6.0s（A nightly+full 6.05 / B stable+plain 5.77）
  - release opt-3 ~6.3s（C 6.30）
  - 構成差 0.5s 程度 ＝ 体感差は出ない（**`./run release` の体感が dev とほぼ変わらない**根拠）
- **true cold（sccache 空、節目）= 10〜20s** ── ここで構成差が大きく出る:
  - dev nightly+full: ~10.6s（`-Z threads` が並列フロントで効く）
  - dev stable+plain: ~12.8s
  - release stable+opt-3: ~20.1s（opt-3 LLVM 仕事を sccache が返せない）
- **warm/cold の分かれ目は「sccache キャッシュキーの世代交代があったか」**: 新規 clone / sccache パージ /
