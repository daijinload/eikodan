...
```
自動化したい場合は任意で pre-push フックを入れる（fastweb変更を含む push でだけ走る）:
```sh
ln -sf ../../fastweb/assets/hooks/pre-push .git/hooks/pre-push   # リポジトリ直下で
```
...
assets/           CSSゲート一式: input.css(追跡) + semgrep/・check-css.sh・hooks/(追跡) +
...
- push前ゲート（クリーンビルド + semgrep）: `assets/check-css.sh` / `assets/semgrep/` / `assets/hooks/pre-push`
