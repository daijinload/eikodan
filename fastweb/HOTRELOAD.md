...
## ★ `--watch` は追記的 ── 消したクラスは即パージされない（が、基本は気にしなくてよい）

`--watch` の差分ビルドはクラスを**足す**のは即時（数十ms、ウォームなら数ms）だが、テンプレから
クラスを**消しても**そのCLIセッション中は `app.css` から消えない（差分エンジンが既出のクラスを保持するため）。

**結論: 日常では気にしなくてよい。**
- **追加は即・正しく反映**される（作業のほとんどはこれ）。消しても残るだけで実害はなく、むしろ
  編集途中でスタイルが消えてチラつかない。一度消したクラスを**戻しても**残っているので即適用される。
- 残るのは `app.css` の使われない死にルールが少し増えるだけ。それも**次のクリーンビルドで消える**
  （ウォッチャ再起動／`--watch` なしのワンショット生成／納品の **release ビルド**）。

**唯一の注意:** `--watch` 中は「**消える / パージされる**」方向は信用できない。
「開発＝本番」でパージ崩れを先に捕まえる狙い（クラスを動的合成や `.rs` に追い出すと本番で消える等）は、
**削除・リファクタ直後は残りルールに隠れて見えない**ことがある。その一点だけはクリーンビルドで確かめる。
