...
/// 全ハンドラで共有する状態。テンプレートのオートリローダと DB プールを保持する。
/// `PgPool` は内部が `Arc` なので clone は安価（接続は使い回される）。
#[derive(Clone)]
pub struct AppState {
...
impl AppState {
    ...
    /// 横断検索するローダと、共有 DB プールから状態を構築する。debugビルドでは
    /// テンプレのファイル変更を監視し、保存のたびに再読み込みする（再コンパイル不要）。
    ///
    /// `css_built` は base.html のCSS切替に使う実行時フラグ（true=CLI生成の /static/app.css、
    /// false=CDN）。全テンプレ共通の表示分岐なのでグローバルに載せる（各ハンドラのctxに通さない）。
    pub fn new(template_dirs: Vec<PathBuf>, pool: PgPool, css_built: bool) -> Self {
    ...
    /// 共有 DB プールへの参照。feature のサービス層がクエリ実行に使う。
