...
| 14  | **`#[cfg(test)] mod tests`（実装ファイル内テスト）**    | 実装とテストが同じ diff に並ぶ。人間が行レビューしない前提ではノイズ。加えてテスト時にクレートを2回コンパイルする                                                                                           | [`../testing.md`](../testing.md) §1〜2                             |
| 15  | **`hoge.test.rs` というファイル名**                     | cargo が受け付けない（`invalid character '.' in crate name`）。`[[test]]` 明示で回避はできるが、仕様1本ごとに Cargo.toml 編集が要る                                                                         | [`../testing.md`](../testing.md) §3                                |
| 16  | **`insta`（スナップショットテスト）**                   | 実装の出力から期待値を自動生成する ＝「テストは実装より先に存在する契約」と正面から矛盾する                                                                                                                 | [`../testing.md`](../testing.md) §6                                |
| 17  | **`mockall`**                                           | 何も検証しないテストを生む。trait 抽象を要求して生成型をそのまま流す設計を壊す                                                                                                                              | [`../testing.md`](../testing.md) §6                                |
| 18  | **`#[sqlx::test]`**                                     | sqlx の `macros` feature が要り #7 に抵触。migration 命名も Flyway（`V{ts}__{desc}.sql`）と噛み合わない                                                                                                     | [`../testing.md`](../testing.md) §6                                |
| 19  | **DB テストの厳密な等値 assert**                        | `counter` は 1 行しか無く並列テスト・開発サーバが同じ行を触る。単調増加で検証し、必要になったら Executor 注入（案A）を採る                                                                                  | [`../testing.md`](../testing.md) §7                                |
...
| **仕様 md とテストの同名ペア配置**                                    | 実装とテストを diff 上で完全に分離し、代わりに仕様とテストを隣に置く。`tests/*.rs` が別クレートになる Rust の仕様上「pub API しか触れない」が強制されるので相性が良い                    | [`../testing.md`](../testing.md)                     |
