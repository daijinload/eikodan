...
[profile.dev.package."*"]
opt-level = 0
codegen-backend = "llvm"             # 依存は LLVM（codegen-backend は据え置き / opt-level は ⑥ で 0 に統一）
```
...
3. **lastshot 固有**: 自前クレートが 50〜200 行と小さく、codegen の絶対量自体が微小。さらに依存は `codegen-backend = "llvm"` 強制なので Cranelift は走らない（依存の opt-level は ⑥ で 0 に揃えたので codegen の重さ自体も小さい）。
...
codegen-backend = "llvm"               # codegen-backend は据え置き（cranelift 切替は別途検証）
```
...
- **§⑤ Cranelift**: 「opt=0 では LLVM が fast path」「依存は llvm 強制で Cranelift は走らない」は引き続き有効
  （codegen-backend は据え置き）。依存が opt=0 になったぶん codegen の絶対量はさらに小さい。
