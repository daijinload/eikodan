...
dev ループの評価軸は**反復速度**だけ・本番動作速度は `[profile.release]` が opt-3 + LTO で担保する、と
profile の役割を明確に分ける。
...
- 動作便益(+17% throughput / +0.2ms p50)は手元の操作頻度では**体感ゼロ**。そもそも **dev で負荷試験はやらない**
  ── 本番速度を見たければ `cargo build --release`（opt-3 + LTO）で別 profile を焼くのが筋。dev profile で
...
- **sccache**: 「重い依存をキャッシュから返す」効果は依然あるが、依存ビルド自体が opt=0 で元より軽いので
  **当時の数値（[`fastweb/BENCHMARK.md` ④](../fastweb/BENCHMARK.md)）ほど劇的ではなくなる可能性が高い**
  （④は dep=opt3 前提で計測。再計測は未実施）。
