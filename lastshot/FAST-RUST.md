...
1. **dev profile の opt-level 非対称（自前=0 / 依存=3）— 効いてる本命** — `[profile.dev]` で自前=0
   （毎回再コンパイル → コンパイル時間最短）、`[profile.dev.package."*"]` で依存=3（一度きり → 実行を速く・
   初回コストは sccache が抱える）に分けて、ビルド時間と実行時間の両方を稼ぐ。`codegen-backend` も同じ
   非対称性で自前=`cranelift` / 依存=`llvm`。これが**本命で**、下の nightly 系（threads / Cranelift）は
   この上に乗る伸び代の保険。詳細は [`COLD-START.md` §⑥](./COLD-START.md)。
