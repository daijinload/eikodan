...
4. **sccache で重い依存をキャッシュ** — 依存（axum / tokio / buffa…）をキャッシュから返して
   ...
   `CARGO_INCREMENTAL=0` まで切れば規模に関わらずフル再ビルドは -40〜52% 速くなるが、日常ループへの税
   (check +0.1〜0.2s / build +0.2〜0.5s、規模が増えるほど増える) を払う。**lastshot 規模では局所的に少し速くなる
   /遅くなるの誤差圏で、最大の旨味は CI/共有キャッシュ**（マシンをまたいで依存を使い回す）。
