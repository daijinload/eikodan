...
1. **dev profile の opt-level 非対称（自前=0 / 依存=3）— dev でも実行を速く** — Rust の dev デフォルトは
   依存まで `opt-level=0` で axum/tokio などが最適化なしで走り、dev 中の体感が release より明確に遅い。
   コスト構造の非対称性を使い `[profile.dev]` で自前は `opt-level=0`（毎回再コンパイル → コンパイル時間最短）、
   `[profile.dev.package."*"]` で依存は `opt-level=3`（一度きり → 実行を速く・初回コストは下の sccache が
   キャッシュ）に分ける。`codegen-backend` も同じ非対称性で自前=`cranelift` / 依存=`llvm`。配線は
   [`Cargo.toml`](./Cargo.toml) の `[profile.dev]`。
2. **開発時は Rust nightly を使う**（`rust-toolchain.toml` で固定。本番は stable）。nightly 限定の高速化を **2つ** opt-in:

   | 機能                                       | 何をする                                                                 | 効くシナリオ                                                       |
   | ------------------------------------------ | ------------------------------------------------------------------------ | ------------------------------------------------------------------ |
   | **`codegen-backend = "cranelift"`**        | 自前クレートの **コード生成段** を LLVM より高速化（依存は LLVM 維持）   | codegen が支配項になる構成（自前クレートが大きい / 高 opt-level dev など） |
   ...
   フル構成と素 stable の差は ±5% 以内）。今は効かないが、**大きいプロジェクトに育てば効くはず**の **保険として残す**:
   `-Z threads` の方は「巨大1クレートでフルビルド約2倍」が `fastweb/BENCHMARK.md` ③ で実証済み、
   Cranelift の方は理屈上 codegen 支配な構成で効くはずだが lastshot ではまだ盤面が来ていない。
...
4. **sccache で重い依存をキャッシュ** — `opt-level=3` でビルドされる重物（axum / tokio / buffa…）をキャッシュから返す。
