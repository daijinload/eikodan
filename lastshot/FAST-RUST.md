...
1. **dev profile は全クレート opt-level=0（フルビルドを最小化）** — `[profile.dev]` 自前=0 / 
   ...
   release(opt3+LTO) でやるので不要**。dev profile の評価軸は反復速度のみ・動作速度は release が担保する、と
   profile の役割を明確に分けた。`codegen-backend` は引き続き自前=`cranelift` / 依存=`llvm`
   （cranelift 切替は別途検証）。実測根拠は [`fastweb/BENCHMARK.md`](../fastweb/BENCHMARK.md) ⑤。
2. **開発時は Rust nightly を使う**（`rust-toolchain.toml` で固定。本番は stable）。nightly 限定の高速化を **2つ** opt-in:

   | 機能                                       | 何をする                                                                 | 効くシナリオ                                                       |
   | ------------------------------------------ | ------------------------------------------------------------------------ | ------------------------------------------------------------------ |
   | **`codegen-backend = "cranelift"`**        | 自前クレートの **コード生成段** を LLVM より高速化（依存は LLVM 維持）   | codegen が支配項になる構成（自前クレートが大きい構成。dev は全クレート opt-level=0 なのでフロント律速だが、それでも保険として残す） |
...
4. **sccache で重い依存をキャッシュ** — 依存（axum / tokio / buffa…）をキャッシュから返して
   ...
   ※ ④の数値は当時の dep=opt3 で計測したもの。dev を全クレート opt-level=0 に揃えた今は依存ビルド自体が
   軽いので sccache の旨味は当時より小さくなる可能性が高い（再計測未実施）。
