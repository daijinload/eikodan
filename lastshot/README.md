...
./run browser             # ブラウザ駆動テスト（Playwright / M2 で追加予定）
...
```sh
./run up                  # app + postgres を docker compose 同一網で起動（VM税なし）
...
lastshot/
  ...
  browser/             ブラウザ駆動テスト（Playwright / M2 で追加予定）
  compose.yml          app + postgres 同一網（M3 で追加予定）
  .github/workflows/   CI（M3 で追加予定）
```
